...
import { Clock, Download, X, Users, AlertTriangle, UserCheck, UserX, Edit, Trash } from 'lucide-vue-next';
...
const attendanceToDelete = ref<Attendance | null>(null);
        ...
        <Dialog v-model:open="showOverrideModal">
            <DialogContent class="sm:max-w-[425px]">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Modify attendance record for {{ selectedAttendance?.user?.name }} on {{ formatDate(selectedAttendance?.date || '') }}
                    </DialogDescription>
                </DialogHeader>
                <form @submit.prevent="submitOverride" class="space-y-4">
                    <div class="space-y-2">
                        ...
                        <TimeInput
                            ...
                            class="w-full"
                        />
                         <p v-if="overrideForm.errors.clock_in" class="text-sm text-red-500">
                            {{ overrideForm.errors.clock_in }}
                        </p>
                    </div>
                    <div class="space-y-2">
                        ...
                        <TimeInput
                            ...
                            class="w-full"
                        />
                        <p v-if="overrideForm.errors.clock_out" class="text-sm text-red-500">
                            {{ overrideForm.errors.clock_out }}
                        </p>
                    </div>
                    <div class="space-y-2">
                        ...
                        <Input
                            ...
                            max="480"
                        />
                        <p v-if="overrideForm.errors.break_minutes" class="text-sm text-red-500">
                            {{ overrideForm.errors.break_minutes }}
                        </p>
                    </div>
                    <div class="flex items-center gap-2">
                        <input
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        ...
                        for <strong>{{ attendanceToDelete?.user?.name }}</strong> on <strong>{{ attendanceToDelete?.date }}</strong>.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
                    <AlertDialogCancel>Cancel</AlertDialogCancel>
                    <AlertDialogAction class="bg-destructive text-destructive-foreground hover:bg-destructive/90" @click="deleteAttendance">
                        Delete
                    </AlertDialogAction>
                </AlertDialogFooter>
            </AlertDialogContent>
